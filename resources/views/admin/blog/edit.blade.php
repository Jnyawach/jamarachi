        ...
        <div class="row">
            <div class="col-11 mx-auto">
                ...
                <form action="{{route('blog.update',$post->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <?php $tag=$post->tag->pluck('name')->implode(',') ?>
                    <div class="form-group mt-3">
                        <label class="control-label" for="tag">Tags</label>

                        <input type="text" class="form-control @error('tag') is-invalid @enderror" name="tag" value="{{$post->tag->pluck('name')->implode(',')}}"
                               required autocomplete="tag" autofocus style="width: 700px;">
                        <small>Enter tags and separate each by a comma for instance: smart phones,coolers,earphones</small>
                        @error('tag')
                        <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
